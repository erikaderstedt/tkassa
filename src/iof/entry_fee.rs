...
use crate::iof::{numeric_contents,textual_contents,IOFXMLError,year_from_date_string};
...
impl TryFrom<&Element> for EntryFee {
    ...
    fn try_from(element: &Element) -> Result<Self, Self::Error> {
        let id: u64 = numeric_contents(element, "EntryFeeId")
            .ok_or("Entry fee id missing or malformed!")?;
        let name = textual_contents(element, "Name")
            .ok_or("Entry fee name missing!")?;
        let amount: f64 = numeric_contents(element, "Amount")
            .ok_or("Entry fee amount missing or malformed!")?;
        ...
        Ok( EntryFee { id, name, amount, operator, from_year_of_birth, to_year_of_birth } )
    }
}

impl EntryFee {

    pub fn paid_fees_from_fee_ids(applicable_fee_ids: &Vec<u64>, event_fees: &Vec<EntryFee>) -> (f64, f64) {
        applicable_fee_ids.iter().fold((0f64,0f64), |acc, fee_id| -> (f64,f64) {
            let fee = event_fees.iter().find(|event_fee| event_fee.id == *fee_id).expect("Event fee id not found!");
